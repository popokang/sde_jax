...
def init_model(rng, config):
  ...
  print("Keys in variables:", variables.keys())
  print("Type of variables['params']:", type(variables.get('params')))
...
def get_model_fn(model, params, states, train=False):
  ...
  def model_fn(x, labels, rng=None):
    if train:
      return states.apply_fn({'params': params}, x, labels, train=True, rngs={'dropout': rng})
    else:
      return states.apply_fn({'params': params}, x, labels, train=False)
    # return states.apply_fn(
    #     {'params': states.ema_params}, x, labels, train=False
    # ) #forward pass / Flax에서는 params가 모델 내부에 저장x, apply()를 호출할 때 입력해야 함.
    
    # return states.apply_fn({'params': params}, x, labels, train, rngs={'dropout': states.key})
